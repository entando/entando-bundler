...
async function generateThumbnail(options) {
    return new Promise((resolve, reject)=>{
        if(options.thumb_file) {
            base64Img.base64(options.thumb_file, (err, data) => {
                if (err) {
                    reject(err);
                }
                resolve(data);
            })
        } else if(options.thumb_url) {
            base64Img.requestBase64(options.thumb_url, (err, res, body) => {
                if (err) {
                    reject(err);
                }
                resolve(body);
...
async function convertToEntandoDeBundle(modules, options) {
    ...
    if (options.thumb_file || options.thumb_url) {
