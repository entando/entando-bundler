...
const componentResponseProcessor = {
  page: async (response, token) => {
    ...
    // rename homepage to landing during mapping to avoid conflicts when installing this bundle
    const schema = {
      'code': [{key: 'code', transform: v => v === 'homepage' ? 'landing' : v}],
      'parentCode': [{key: 'parentCode', transform: v => v === 'homepage' ? 'landing': v}],
      'titles': 'titles',
      'pageModel': 'pageModel',
  ...
  pageModel: async (response, token) => {
    ...
    return pageModels.map(p => objectMapper(p, schema));
  },
  ...
  widget: async (response, token) => {
    ...
    return widgets.filter(w => w.customUi !== undefined && w.group !== null).map(w => objectMapper(w, schema));
  },
  ...
  pageModel: async (components) => {
    ...
    const resources = await Promise.all(components.map(async (pageModel) => {
      ...
      if (pageModel.hasOwnProperty('template') && pageModel.template !== null) {
        const regex = new RegExp('"<@wp.resourceURL[\\n\\t\\r\\s]*/>(?:[^"]+/)?(/static/[^"]+)"', 'g');
        while (found = regex.exec(pageModel.template)) {
  ...
  widget: async (components) => {
    ...
    const resources = await Promise.all(components.map(async (widget) => {
      ...
      if (widget.hasOwnProperty('customUi') && widget.customUi !== null) {
        const regex = new RegExp('"<@wp.resourceURL[\\n\\t\\r\\s]*/>(?:[^"]+/)?(/widget/[^"]+)"', 'g');
        while (found = regex.exec(widget.customUi)) {
          resources.push(found[1]);
        }
      }
...
const resourceLinkRegex = new RegExp('(?:[.]+/)?((?:widgets)/[^"]+)', 'g');
  ...
  widget: (type, location, code, components) => {
    const processedComponents = components.map(widget => {
      if (widget.hasOwnProperty('configUi') && widget.configUi !== null && widget.configUi.hasOwnProperty('resources')) {
        ...
        widget.configUi.resources = widget.configUi.resources.map(r => r.replace(resourceLinkRegex, `${code}/$1`));
      }
...
async function generateBundle (options, components) {
  ...
  const uniqueUsedFiles = resources.reduce((a, b) => a.concat(b.filter(i => !a.includes(i))), []);
...
async function saveComponentsDescriptor(type, location, bundle_code, componentIds) {
  ...
  const descriptors = descriptorProcessor(type, location, bundle_code, components);
...
async function saveResources(location, resourcesPaths) {
  ...
  const components = resourcesPaths.map(r => componentCache['resource'].find(c => c.path === r));
