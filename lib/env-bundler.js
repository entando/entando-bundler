...
const componentResponseProcessor = {
  page: async (componentsToFetch, token) => {
    ...
    const nestedPages = await recursivelyTraverseComponentsByCodeAndParentCode(pages, 'page', token);
  ...
  asset: async (componentsToFetch, token) => {
    ...
    return assets
      ...
      .map(a => {
        // handle image path vs. file path
        a.path = a.path !== undefined ? a.path : a.imagePath;
        delete a.imagePath;
        return a;
      })
      .filter(a => a.path !== undefined);
  },
  ...
  category: async (componentsToFetch, token) => {
    ...
    const nestedCats = await recursivelyTraverseComponentsByCodeAndParentCode(categories, 'category', token);
...
const assetComponentDescriptorProcessor = async function(type, location, code, components) {
  const typePlural = typeToPlural(type);
  const codeExtractor = componentCodeExtractors[type];
  ...
  return Promise.all(components.map(async (component) => {
      ...
      const filePathSegment = `${isProtected ? `${component.group}/` : ''}${component.fileName}`;
...
async function recursivelyTraverseComponentsByCodeAndParentCode(components, type, token) {
  ...
  const nestedComps = await recursivelyTraverseComponentsByCodeAndParentCode(childrenComps, type, token);
