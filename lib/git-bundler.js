...
const { generateModuleName, generateThumbnail, validateModuleName, bundleTypes } = require('./de_bundle');
...
function getDescriptorData () {
  ...
  return fs.promises.readFile(`${DESTINATION_DIR}/descriptor.yaml`, 'utf8')
    ...
    .then(descriptorData => {
      const components = ((descriptorData.components &&
      ...
      .then(([descriptorData, repositoryData]) => {
        ...
        Promise.all([thumbnailPromise, ...tagsCommitIdPromises])
          .then(([thumbnail, ...tags]) => {
            ...
            labels.labels['bundle-type'] = descriptorData['bundle-type'] || bundleTypes.STANDARD_BUNDLE;
...
const fromGitQuestions = [
  {
    ...
    validate: (answer) => {
      if ((answer.startsWith('https://') || answer.startsWith('ssh://git@') || answer.startsWith('git@')) && answer.endsWith('.git')) {
        return true;
      }
      return "You need to provide a valid git repository URL, which should start with one of ('https://','ssh://git@','git@') and and with '.git'";
    },
  },
  {
    type: 'list',
  ...
  },
  {
    type: 'list',
    name: 'dryRun',
    message: 'What do you want to do with the generated custom resource?',
    choices: [
      { value: 'print', name: 'Print the result in the console' },
      { value: 'deploy', name: 'Deploy the custom resource on the default kubernetes cluster' },
    ],
    filter: (input) => input === 'print',
  },
];

async function interactiveSession () {
  return await inquirer.prompt(fromGitQuestions);
}
