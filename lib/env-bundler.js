...
const componentResponseProcessor = {
  page: async (componentsToFetch, token) => {
    let pages = componentsToFetch;

    const nestedPages = await recursivelyTraversePages(pages, token);
  ...
  pageModel: async (componentsToFetch, token) => {
  ...
  fragment: async (componentsToFetch, token) => {
  ...
  widget: async (componentsToFetch, token) => {
  ...
  contentType: async (componentsToFetch, token) => {
    const contentTypes = await Promise.all(componentsToFetch.map(async (contentType) => {
      ...
      contentType.attributes = res.data.payload.attributes.map(a => {
        if (a.code === 'title' && a.roles.length === 0) {
          console.log('###### ' + objectMapper(a, schema));
        }
        return objectMapper(a, schema);
      });
  ...
  contentModel: async (componentsToFetch, token) => {
  ...
  content: async (componentsToFetch, token) => {
  ...
  asset: async (componentsToFetch, token) => {
  ...
  plugin: async (componentsToFetch, token) => {
  ...
  group: async (componentsToFetch, token) => {
    return componentsToFetch;
  },
  category: async (componentsToFetch, token) => {
    const categories = componentsToFetch;
    const schema = {
      code: 'code',
      parentCode: 'parentCode',
      titles: 'titles',
  ...
  resource: async (componentsToFetch, token) => {
    console.log('Starting File Browser traversion...');
    const files = await recursiveTraverseFileTree(componentsToFetch, token);
    return files;
  },
  label: async (componentsToFetch, token) => {
    return componentsToFetch;
  },
  language: async (componentsToFetch, token) => {
...
async function getComponents (type, componentsToFetch) {
  ...
  const components = await componentResponseProcessor[type](componentsToFetch, token);
...
async function recursivelyTraversePages (pages, token) {
  let childrenPages = await Promise.all(
    pages.map(async (page) => {
      const childrenResponse = await axios.get(apiUrlTable.page, {
        params: {
  ...
  // if no nested pages => exit recursion
  if (childrenPages.length === 0) {
    return pages;
  }
  // flat nested pages arrays
  childrenPages = childrenPages.reduce((pages1, pages2) => pages1.concat(pages2));
  // traverse nested pages recursively
  const nestedPages = await recursivelyTraversePages(childrenPages, token);
  return childrenPages.concat(nestedPages);
}
