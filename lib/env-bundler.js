...
const componentResponseProcessor = {
  page: async (componentsToFetch) => {
    ...
    // Get only the published page details or all if --force flag is used
    const currentPagesList = forceExportAllComponents ? pages : pages.filter(p => p.status !== 'unpublished');

    pages = await Promise.all(currentPagesList.map(async (page) => {
  ...
  pageModel: async (componentsToFetch) => {
    ...
    const currentPageModelsList = forceExportAllComponents ? pageModels : pageModels.filter(p => p.configuration && p.configuration.frames.length > 0);

    return currentPageModelsList.map(p => objectMapper(p, schema));
  },
