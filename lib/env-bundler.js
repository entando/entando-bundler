...
const componentResponseProcessor = {
  ...
  resource: async (response, token) => {
    ...
    files = await recursiveTraverseFileTree(response.data.payload, token);
    return files;
  },
};
...
async function recursiveTraverseFileTree(root, token) {
  const root_traversed = await Promise.all(root.map(async (resource) => {
    if (resource.directory === true) {
      ...
      const response = await axios.get(apiUrlTable['resourceDetails'].replace('currentPath=', 'currentPath=' + resource.path), {
        headers: { Authorization: `Bearer ${token}` }
      });
...
async function setupEnvironment(options) {
  ...
  apiUrlTable = {
    ...
    resourceDetails: `${coreBaseApi}/api/fileBrowser/file?protectedFolder=false&currentPath=`,
  };
}
