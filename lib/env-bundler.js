...
const collectQuestions = [
  {
    ...
    choices: [
      { name: 'All components no plugins', value: 'all' },
      { name: 'All components with plugins', value: 'allWithPlugins' },
  ...
  {
    when: (answers) => answers.componentType !== 'all' && answers.componentType !== 'allWithPlugins',
    type: 'checkbox',
    name: 'components',
...
async function inquireComponents () {
  ...
  } while (answers.componentType !== 'all' && answers.componentType !== 'allWithPlugins' && answers.addMoreComponents);
}
...
async function setupEnvironment (options) {
  ...
  //plugins included only if specified
  if (options.includePlugins === true) {
    ALL_TYPES.push('plugin');
  }
