...
const componentResponseProcessor = {
  page: async (componentsToFetch) => {
    ...
    // extract homepage
    const resHomepage = await axios.get(apiUrlTable.pageDetails.replace('{code}', 'homepage'), {
      headers: { Authorization: `Bearer ${token}` },
    });
    pages.unshift(resHomepage.data.payload); // Add to beginning of array
  ...
  fragment: async (componentsToFetch) => {
    ...
    const currentFragmentsList = forceExportAllComponents ? fragments : fragments.filter(f => !f.code.includes('-'));

    return currentFragmentsList
  ...
  widget: async (componentsToFetch) => {
    ...
    const currentWidgetsList = forceExportAllComponents ? widgets : widgets.filter(w => !w.locked && w.group);
  ...
  asset: async (componentsToFetch) => {
    ...
    const currentAssetsList = assets.map(a => objectMapper(a, schema));

    return forceExportAllComponents ? currentAssetsList : currentAssetsList.filter(a => a.fileName !== undefined);
  },
...
const resourceExtractorByType = {
  ...
  pageModel: async (components) => {
    ...
    const resources = await Promise.all(components.map(async (pageModel) => {
      const resources = [];
      if (hasProperty(pageModel, 'template') && pageModel.template !== null) {
        let found;
  ...
  fragment: async (components) => {
    ...
    const resources = await Promise.all(components.map(async (fragment) => {
      const resources = [];
      if (hasProperty(fragment, 'guiCode') && fragment.guiCode !== null) {
        let found;
  ...
  widget: async (components) => {
    ...
    const resources = await Promise.all(components.map(async (widget) => {
      const resources = [];
      if (hasProperty(widget, 'configUi') && widget.configUi !== null && hasProperty(widget.configUi, 'resources')) {
        resources.push.apply(resources, widget.configUi.resources);
      }

      if (hasProperty(widget, 'customUi') && widget.customUi !== null) {
        let found;
...
const processContentAttribute = function (attribute) {
  ...
  if (hasProperty(attribute, 'values') && attribute.values !== null) {
    for (var language in attribute.values) {
      const value = attribute.values[language];
      if (hasProperty(value, 'type') && (value.type === 'image' || value.type === 'file')) {
  ...
  if (hasProperty(attribute, 'compositeelements') && attribute.compositeelements !== null) {
    attribute.compositeelements = attribute.compositeelements.map(a => processContentAttribute(a));
  }

  if (hasProperty(attribute, 'elements') && attribute.elements !== null) {
...
const assetComponentDescriptorProcessor = async function (type, location, components) {
  const typePlural = typeToPlural(type);
  const codeExtractor = componentCodeExtractors[type];
  ...
  const processedComponents = await Promise.all(components.map(async (component) => {
    ...
    try {
      ...
      fs.writeFileSync(`${directory}/${component.name}`, buffer);
    } catch (error) {
      const assetInfo = (({ correlationCode, description, fileName, name }) => ({ correlationCode, description, fileName, name }))(component);
      console.log(`Error processing asset ${JSON.stringify(assetInfo)}, the asset will be skipped, error details:\n${error}`);
      return;
    }
    ...
    fs.writeFileSync(`${location}/${descriptorName}`, yaml.safeDump(component, { lineWidth: 1000000 }));

    return descriptorName;
  }));

  return processedComponents.filter(c => c !== undefined);
};
...
const componentDescriptorProcessors = {
  ...
  asset: (type, _, location, components) => {
    return assetComponentDescriptorProcessor(type, location, components);
  },
...
async function saveComponentsDescriptor (type, location, bundleCode, componentIds) {
  ...
  const descriptors = await descriptorProcessor(type, location, bundleCode, components);
...
async function setupEnvironment (options) {
  ...
  apiUrlTable = {
    ...
    pageDetails: `${coreBaseApi}/api/plugins/seo/pages/{code}`,
  ...
  try {
    ...
    if (error.isAxiosError && error.message.includes('certificate')) {
      throw new Error('Invalid certificate detected. Use the --no-tls-verify flag to skip TLS validation');
    }
