...
describe('Bundle convertion', () => {
    it("Should take the name from first module if no custom name is provided", async () => {
        let b = await bundle.convertToEntandoDeBundle(mockedNpmResponses.allVersions, {}); 
        expect(b.metadata.name).toBe("entando-menu");
    })

    it("Should use custom name", async () => {
        let b = await bundle.convertToEntandoDeBundle(mockedNpmResponses.allVersions, {'name': 'my-entando'}); 
    ...
    it("Should generate details based on module infos", async () => {
        ...
        let b = await bundle.convertToEntandoDeBundle(allVersions, {});
        let expectObject = {
            name: "@entando/menu",
    ...
    it("Should use the provided namespace", async () => {
        let b = await bundle.convertToEntandoDeBundle(mockedNpmResponses.allVersions, {"namespace": "test-namespace"});
