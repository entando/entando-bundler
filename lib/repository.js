...
function cloneRepository (options = {}) {
  ...
  return new Promise((resolve, reject) => {
    if (repository) {
      exec(
      `cd ${CLONE_DIR_ROOT} && mkdir ${CLONE_DIR} && cd ${CLONE_DIR} && git clone ${repository} .`,
      { windowsHide: true },
      (err, stdout, stderr) => {
        if (err) {
          reject(err);
...
function removeRepository (bundle) {
  return new Promise((resolve, reject) => {
    exec(
      `cd ${CLONE_DIR_ROOT} && rm -fr ${CLONE_DIR}`,
      { windowsHide: true },
      (err, stdout, stderr) => {
        if (err) {
          reject(err);
...
function getDescriptorData (options) {
  ...
  const workingPath = repository ? `${CLONE_DIR_ROOT}/${CLONE_DIR}` : repositoryPath;
...
function getRepositoryData (options = {}) {
  ...
  return new Promise((resolve, reject) => {
    const tagListPromise = new Promise((resolve, reject) => {
      const workingPath = repository ? `${CLONE_DIR_ROOT}/${CLONE_DIR}` : repositoryPath || null;
      exec(
